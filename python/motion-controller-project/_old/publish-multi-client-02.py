...
def publish_temperature(client):
    while running:
        client.publish("sensehat/env/pressure", str(random.uniform(950, 1050)))
        time.sleep(2)  # Publish every 2 seconds

def publish_gyroscope(client):
    while running:
        client.publish("sensehat/imu/gyroscope", str(random.uniform(-1.0, 1.0)))
