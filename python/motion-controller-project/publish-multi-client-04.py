...
for i in range(6):
    init_client("client_{i}", protocol, host, port, username, password, tls)

for client in clients:
    client.loop_start()


init_thread(publish, clients[0], topics["humidity"], 5.0, getData.getHumidityData, sense, uuidDevice, dateTime)
init_thread(publish, clients[1], topics["pressure"], 5.0, getData.getPressureData, sense, uuidDevice, dateTime)
init_thread(publish, clients[2], topics["accelerometer"], 1.0, getData.getAccelerometerData, sense, uuidDevice, dateTime)
init_thread(publish, clients[3], topics["gyroscope"], 1.0, getData.getGyroscopeData, sense, uuidDevice, dateTime)
init_thread(publish, clients[4], topics["magnetometer"], 1.0, getData.getMagnetometerData, sense, uuidDevice, dateTime)
